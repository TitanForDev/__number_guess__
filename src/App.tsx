    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans text-slate-800 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 opacity-40 pointer-events-none">
        {[...Array(6)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute rounded-full bg-indigo-100 blur-3xl"
            style={{
      ...
      {/* Header Navigation */}
      <nav className="h-16 px-4 md:px-12 border-b border-slate-200 bg-white/80 backdrop-blur-md flex items-center justify-between shrink-0 z-10">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold">N</div>
          <span className="text-xl font-bold tracking-tight text-slate-900">猜數字<span className="text-indigo-600">.挑戰</span></span>
        </div>
        <div className="flex items-center gap-4 md:gap-6">
          <div className="hidden sm:flex flex-col items-end">
            <span className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">最佳紀錄</span>
            <span className="text-sm font-semibold">{bestScore ? `${bestScore} 次猜測` : '--'}</span>
          </div>
          <div className="hidden sm:block h-8 w-px bg-slate-200"></div>
          <button 
            onClick={initGame}
            className="px-4 py-2 bg-slate-900 text-white text-[10px] sm:text-xs font-bold rounded-full uppercase tracking-widest hover:bg-slate-800 transition-colors"
          >
            重置遊戲
          </button>
        </div>
      ...
      <main className="flex-1 overflow-auto p-4 md:p-12 flex items-center justify-center z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 w-full max-w-6xl items-stretch">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6 order-2 lg:order-1">
            {/* History Card */}
            <div className="bg-white/80 backdrop-blur-md border border-slate-200 rounded-3xl p-6 md:p-8 shadow-sm flex-1 min-h-[300px] flex flex-col">
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6">猜測歷史紀錄</h2>
              <div className="space-y-3 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
                <AnimatePresence initial={false}>
                  {guesses.length === 0 ? (
                    <div className="text-sm text-slate-300 italic py-4">目前尚無紀錄。</div>
                  ) : (
                    guesses.map((g, i) => (
                      <motion.div
                        ...
                        className={`flex items-center justify-between p-3 rounded-xl border ${
                          g.result === 'correct' ? 'bg-indigo-50 border-indigo-100 shadow-indigo-50 shadow-sm' : 'bg-slate-50 border-slate-100'
                        } transition-shadow`}
                      >
                        <span className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                          g.result === 'correct' ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-600'
                        }`}>
            ...
            <div className="bg-indigo-900 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden shrink-0">
              <div className="relative z-10">
                <h3 className="text-xs font-bold text-indigo-300 uppercase tracking-widest mb-1">剩餘機會</h3>
              ...
              <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-indigo-800 rounded-full opacity-50 blur-xl"></div>
            </div>
          </div>
          ...
          <motion.div 
            ...
            className={`lg:col-span-8 bg-white border border-slate-200 rounded-[40px] shadow-2xl p-8 md:p-12 flex flex-col items-center justify-center relative min-h-[500px] order-1 lg:order-2 transition-colors duration-300 ${isShaking ? 'bg-rose-50 border-rose-200' : ''}`}
          >
            ...
            <div className="mb-10 text-center">
              <motion.div 
                ...
                className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full border mb-6 ${
                  message.includes('高') ? 'bg-rose-50 text-rose-700 border-rose-100' : 
                  message.includes('低') ? 'bg-blue-50 text-blue-700 border-blue-100' :
                  message.includes('中') ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
            ...
            <div className="w-full max-w-sm">
              <div className="relative mb-8 group">
                <motion.input 
                  ...
                  placeholder="00"
                  className="w-full text-center text-8xl font-black text-slate-900 border-b-4 border-slate-100 focus:border-indigo-600 focus:outline-none bg-transparent py-4 transition-all placeholder:text-slate-100 disabled:opacity-50"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button 
                  ...
                  className="h-16 rounded-2xl bg-slate-100 text-slate-900 font-bold hover:bg-slate-200 transition-colors disabled:opacity-50"
                >
                  清除
                </button>
                <button 
                  onClick={() => handleGuess()}
                  disabled={status !== 'playing' || !currentGuess}
                  className="h-16 rounded-2xl bg-indigo-600 text-white font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all disabled:opacity-50"
                >
                  送出猜測
                </button>
              </div>
            ...
            <AnimatePresence>
              {status !== 'playing' && (
                <motion.div 
                  ...
                  className="absolute inset-0 bg-white/95 backdrop-blur-md rounded-[40px] flex flex-col items-center justify-center z-50 p-8 text-center"
                >
                  <motion.div 
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                  ...
                  <button 
                    onClick={initGame}
                    className="px-10 py-4 bg-slate-900 text-white font-bold rounded-2xl shadow-xl hover:bg-slate-800 active:scale-95 transition-all flex items-center gap-2"
                  >
      ...
      {/* Bottom Bar Info */}
      <footer className="h-16 px-4 md:px-12 border-t border-slate-200 bg-white/50 backdrop-blur-sm flex items-center justify-between text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest shrink-0 z-10">
